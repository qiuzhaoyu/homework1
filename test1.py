...
focus_distance_1 = 30  # 焦距参数1
focus_distance_2 = 35  # 焦距参数2
    ...
    for row in range(num_rows):
        for col in range(num_cols):
            ...
            if weight > 0.0001:  # 忽略小权重
                dx = int((col - x_pos) * focal_length_factor * width / focus_distance_2)  # x方向上的位移
                dy = int(-(row - y_pos) * focal_length_factor * height / focus_distance_1)  # y方向上的位移
...
def on_focus_distance_1_change(val):
    global focus_distance_1
    focus_distance_1 = max(val, 1)  # 避免焦距1为0

def on_focus_distance_2_change(val):
    global focus_distance_2
    focus_distance_2 = max(val, 1)  # 避免焦距2为0
...
def main():
    ...
    # 创建滑动条控制焦距参数
    cv2.createTrackbar('focus_z1', 'HomeWork1', int(focus_distance_1), 100, on_focus_distance_1_change)
    cv2.createTrackbar('focus_z2', 'HomeWork1', int(focus_distance_2), 100, on_focus_distance_2_change)
    ...
    # 创建滑动条控制 z 轴（视角深度）
    cv2.createTrackbar('focal_length', 'HomeWork1', int(focal_length_factor * 10), 100, on_focal_length_factor_change)
